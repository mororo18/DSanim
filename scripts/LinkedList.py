...
class LinkedList(MovingCameraScene):
    def bounds(self, i):
    ...
    def build_element(self, index, label, pos):
        ...
        i = Text("{}".format(index)).scale(0.6).next_to(rect, direction=DOWN)
        l = Text("{}".format(label)).move_to(rect.get_center() - [1, 0, 0])
        c = rect.get_center() + [1, 0, 0]
        return VGroup(rect, i, l, Arrow(start=c, end=c + self.A_SIZE*RIGHT))
    def build_element_single(self, label, pos):
        s = Square().move_to(pos)
        t_label = Text(label).scale(0.6).next_to(s, direction=DOWN)
        return VGroup(s, t_label, Arrow(start=s.get_center(), end=s.get_center() + self.A_SIZE*RIGHT))
    ...
    def get_tudo(self):
        return VGroup(self.head, self.array)

    def push_back(self, e):
        if self.get_size() == 0:
            element = self.build_element(self.get_size(), e, ORIGIN).next_to(self.head)
            ...
            self.play(GrowArrow(self.get_arrow(-2)))
    ...
    def construct(self):
        ...
        self.head = self.build_element_single("Head", ORIGIN)
        self.camera.frame.set(width=self.head.width * 1.5).move_to(self.head.get_center())
