...
class Queue(MovingCameraScene):
   ...
   #        self.remove(arrow) 
    ...
    def build_element(self, index, label, pos):
        ...
        return VGroup(rect, l, Arrow(start=c, end=c + self.A_SIZE*RIGHT))
    ...
    def build_head(self, pos):
        s = Square().move_to(pos)
        t_label = Text("Front").scale(0.6).next_to(s, direction=UP)
        return VGroup(s, t_label, Arrow(start=s.get_center(), end=s.get_center() + self.A_SIZE*DOWN))
    def build_tail(self, pos):
        s = Square().move_to(pos)
        t_label = Text("Back").scale(0.6).next_to(s, direction=DOWN)
    ...
    def front(self):
        if self.get_size() == 0:
            self.print("Fila vazia!")
            return
        self.print("O primeiro item da fila: {}".format(self.get_label(0).original_text))
    ...
    def pop(self):
        ...
        if self.get_size() == 1:
            ...
            self.play(
                self.head.animate.next_to(self.array[1][:-1], direction=UP)
                    )
            front = self.array[0]
            self.array -= front
